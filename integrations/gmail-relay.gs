 ...
 * Taskz gmail-ingest Edge Function, which extracts any commitments you made
 * and queues them for review in the app.
 *
 * Setup
 * -----
 *  1. script.google.com → New project → paste this file in.
 *  2. Fill in INGEST_URL and INGEST_TOKEN below. Both are shown in Taskz
 ...
 * anywhere while the toggle in Taskz is off — the Edge Function checks it
...
function scanSentMail() {
  if (INGEST_URL.indexOf('PASTE_') === 0 || INGEST_TOKEN.indexOf('PASTE_') === 0) {
    throw new Error('Set INGEST_URL and INGEST_TOKEN first — both are in Taskz under ··· → Settings.');
  }
  ...
  for (var t = 0; t < threads.length && messages.length < MAX_MESSAGES; t++) {
    ...
    for (var m = 0; m < threadMessages.length && messages.length < MAX_MESSAGES; m++) {
      const msg = threadMessages[m];

      // An "in:sent" thread also contains the replies you received.
  ...
  if (result.disabled) {
    // The integration is switched off in Taskz. Don't advance the watermark,
    // so whatever was sent while it was off gets picked up when it's on again.
    Logger.log('Gmail integration is switched off in Taskz. Nothing processed.');
    return;
  }
