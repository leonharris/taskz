...
${suffix}`;
  ...
  var supabaseKey = "sb_publishable_9yIm45PraHQbxSwIt2VHkw__xx72VtL";
  ...
  async function saveBoardToSupabase() {
    ...
    const { data: existingBoard } = await supabase.from("boards").select("id").eq("user_id", currentUser.id).single();
    if (existingBoard) {
      await supabase.from("boards").update({ data: boardData, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", existingBoard.id);
    } else {
      await supabase.from("boards").insert({ user_id: currentUser.id, data: boardData });
    }
  }
