(() => {
  ...
  var IroColor = /* @__PURE__ */ (function() {
    ...
    _proto.setChannel = function setChannel(format, channel, value) {
      var _extends22;
      this[format] = _extends({}, this[format], (_extends22 = {}, _extends22[channel] = value, _extends22));
    };
  ...
  var IroComponentWrapper = /* @__PURE__ */ (function(Component) {
    ...
    IroComponentWrapper2.prototype.handleEvent = function handleEvent2(e2) {
      var this$1 = this;
    ...
    function handleInput(x2, y2, inputType) {
      if (inputType === 0) {
        ...
        if (activeHandle !== null) {
          colorPicker2.setActiveColor(activeHandle);
        } else {
          colorPicker2.inputActive = true;
  ...
  var IroColorPicker = /* @__PURE__ */ (function(Component) {
    ...
    IroColorPicker2.prototype.addColor = function addColor(color, index2) {
      if (index2 === void 0) index2 = this.colors.length;
    ...
    IroColorPicker2.prototype.on = function on2(eventList, callback) {
      var this$1 = this;
    ...
    IroColorPicker2.prototype.off = function off2(eventList, callback) {
      var this$1 = this;
      (!Array.isArray(eventList) ? [eventList] : eventList).forEach(function(eventType) {
...
${suffix}`;
  ...
  var supabaseUrl = "https://xkdmdyafikvtmpgumzpv.supabase.co";
  var supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhrZG1keWFmaWt2dG1wZ3VtenB2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAxMTU0NTIsImV4cCI6MjA4NTY5MTQ1Mn0.9PxvvpR5JiOh48xibb-fLFu-zD9EF20c-3s__F6893g";
  var supabase = createClient(supabaseUrl, supabaseKey);
  var currentUser = null;
  var boardLoaded = false;
  var boardDirty = false;
  var lastSavedAt = null;
  var boardChannel = null;
  ...
  async function saveBoardToSupabase() {
    ...
    try {
      ...
      const { data: existingBoard, error: fetchError } = await supabase.from("boards").select("id").eq("user_id", currentUser.id).single();
      ...
      if (existingBoard) {
        ...
        const { error: updateError } = await supabase.from("boards").update({ data: boardData, updated_at: now }).eq("id", existingBoard.id);
        if (updateError) {
        ...
        const { error: insertError } = await supabase.from("boards").insert({ user_id: currentUser.id, data: boardData });
        if (insertError) {
  ...
  async function loadBoardFromSupabase() {
    ...
    try {
      ...
      const { data: board, error } = await supabase.from("boards").select("data").eq("user_id", currentUser.id).single();
  ...
  function purgeOldArchivedTasks(tasks) {
    ...
    return tasks.filter((t2) => new Date(t2.archivedAt) > cutoff);
  }
  ...
  function populateTasksFromData(tasks) {
    ...
    for (let col of tasks) {
      ...
      for (let task of task_items) {
        ...
        listCol[i2].appendChild(task_li);
      }
      i2++;
    }
    activateSortable();
  ...
  function applyFilter(filter) {
    ...
    document.querySelectorAll(".task").forEach((task) => {
      const matches2 = activeFilter === "all" || task.dataset.priority === activeFilter;
      task.classList.toggle("is-filtered-out", !matches2);
    });
  ...
  function subscribeToBoardChanges() {
    ...
    boardChannel = supabase.channel("board-changes").on(
      "postgres_changes",
      {
        event: "UPDATE",
        schema: "public",
  ...
  document.addEventListener("click", (e2) => {
    if (e2.target.id === "sync-reload" || e2.target.closest("#sync-reload")) {
      boardDirty = false;
      hideSyncToast();
      loadBoardFromSupabase();
    }
    if (e2.target.id === "sync-dismiss" || e2.target.closest("#sync-dismiss")) {
      hideSyncToast();
    }
  });
  var colorPicker = new iro_es_default.ColorPicker("#color-picker", {
  ...
  document.addEventListener("click", (e2) => {
    const btn = e2.target.closest(".btn-add-task");
    if (btn) {
  ...
  function createList(blank_col, column_title, column_color) {
    ...
    new sortable_esm_default(board, {
      animation: 300,
      delay: 200,
      delayOnTouchOnly: true,
      touchStartThreshold: 5,
  ...
  document.getElementById("board").addEventListener("click", (e2) => {
    const btn = e2.target.closest(".btn-delete-task");
    if (!btn) return;
  ...
  var addListModal = document.getElementById("modal--add-list");
  ...
  addListModal.addEventListener("click", (e2) => {
    if (e2.target === addListModal) {
      closeModal();
    }
  });
  var taskDetailModal = document.getElementById("modal--task-detail");
  var taskDetailForm = document.getElementById("form--task-detail");
  ...
  var taskDetailPriority = document.getElementById("task-detail-priority");
  var taskDetailDescription = document.getElementById("task-detail-description");
  var activeTask = null;
  document.getElementById("board").addEventListener("click", (e2) => {
    const taskEl = e2.target.closest(".task");
    if (!taskEl) return;
    if (e2.target.closest(".btn-delete-task-wrap")) return;
  ...
  function openTaskDetailModal(taskEl) {
    ...
    columns.forEach((col) => {
      ...
      option2.value = col.id;
      option2.textContent = name;
      if (col === currentColumn) option2.selected = true;
      taskDetailStatus.appendChild(option2);
    });
  ...
  taskDetailModal.addEventListener("click", (e2) => {
    if (e2.target === taskDetailModal) closeTaskDetailModal();
  });
  taskDetailForm.addEventListener("submit", (e2) => {
    e2.preventDefault();
  ...
  var trashModal = document.getElementById("modal--trash");
  ...
  function renderTrashList() {
    ...
    trashList.innerHTML = "";
    archivedTasks.forEach((task, i2) => {
      const date = new Date(task.archivedAt).toLocaleDateString(void 0, { year: "numeric", month: "short", day: "numeric" });
      ...
      <div class="trash-item--actions">
        <button class="btn btn-secondary btn-small btn-restore-task" data-index="${i2}">Restore</button>
        <button class="btn btn-danger btn-small btn-delete-forever" data-index="${i2}">Delete forever</button>
      </div>
    `;
      trashList.appendChild(item);
    });
  }
  function restoreTask(index2) {
    const task = archivedTasks[index2];
    if (!task) return;
    let targetList = null;
    ...
    archivedTasks.splice(index2, 1);
    markDirty();
    renderTrashList();
  }
  function deleteForever(index2) {
    archivedTasks.splice(index2, 1);
    markDirty();
    renderTrashList();
  }
  ...
  trashModal.addEventListener("click", (e2) => {
    if (e2.target === trashModal) closeTrashModal();
  });
  document.getElementById("btn-empty-trash").addEventListener("click", emptyTrash);
  trashList.addEventListener("click", (e2) => {
    const restoreBtn = e2.target.closest(".btn-restore-task");
    if (restoreBtn) {
      restoreTask(parseInt(restoreBtn.dataset.index));
      return;
    }
    const deleteBtn = e2.target.closest(".btn-delete-forever");
    if (deleteBtn) {
  ...
  if (authForm) {
    authForm.addEventListener("submit", async function(e2) {
      e2.preventDefault();
  ...
  document.addEventListener("click", function(e2) {
    if (e2.target.id === "sign-out-btn" || e2.target.closest("#sign-out-btn")) {
      signOut();
    }
  });
  var burgerBtn = document.getElementById("burger-btn");
  var appNavbar = document.querySelector(".app-navbar");
  if (burgerBtn && appNavbar) {
    burgerBtn.addEventListener("click", function(e2) {
      e2.stopPropagation();
  ...
  document.addEventListener("click", function(e2) {
    if (appNavbar && appNavbar.classList.contains("is-open") && !appNavbar.contains(e2.target)) {
  ...
  document.addEventListener("click", function(e2) {
    ...
    if (closeTriggers.some((id) => e2.target.id === id || e2.target.closest("#" + id))) {
